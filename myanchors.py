...
def kmeans(box, k):
    row = box.shape[0]  # 這張圖有幾個bbox
    
    distance = np.empty((row, k))  # 每個bbox的資訊
...
if __name__ == '__main__':
    np.random.seed(0)
    input_shape = [1280, 1280] # 大小最好跟訓練、推論時都設一樣
    anchors_num = 12  # 要有幾個"群"中心
    ...
    print('K-means boxes.')
    cluster, near   = kmeans(data, k = anchors_num)
